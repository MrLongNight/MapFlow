...
- Die frueher genannten Dead-Code-Hinweise sind in den konkret genannten Dateien nicht mehr in derselben Form sichtbar:
  - `crates/vorce-render/src/mesh_buffer_cache.rs` ist heute aktiv im Renderpfad benutzt.
  - in `crates/vorce-ui/src/editors/mesh_editor/*` ist die fruehere pauschale `dead_code`-Lage in dieser Form nicht mehr offensichtlich.
...
- Output-Fenster-Cleanup ist inzwischen implementiert (`crates/vorce/src/orchestration/outputs.rs` entfernt stale windows).
...
- Trotzdem bleiben parallele oder alte Video-Pfade offen:
  - `crates/vorce-render/src/paint_texture_cache.rs`: `PaintType::Video` und `PaintType::Camera` TODOs wurden entfernt und als Legacy-Pfade dokumentiert.
...
- `crates/vorce-ui/src/editors/module_canvas/inspector/source.rs` zeigt weiterhin explizite Hinweise, dass bestimmte Source-Typen nicht voll an die Runtime angeschlossen sind:
  ...
  - LiveInput
  - SpoutInput
- `crates/vorce-media/src/hap_player.rs` sagt weiterhin selbst, dass der Pfad noch eine FFmpeg-Integration als Platzhalter-Luecke hat.
- Default-Feature-Gating ist verbessert:
  - `vorce-io` hat `default = []`
...
- Blend Modes:
  - `crates/vorce/src/orchestration/evaluation.rs` erzeugt weiter `blend_mode_unsupported`.
  - `crates/vorce-ui/src/editors/module_canvas/inspector/capabilities.rs` behandelt praktisch nur `Normal` als wirklich unterstuetzt.
...
- Weiter offen:
  ...
  - im Rendercode werden weiter BindGroups/Uniform-Buffer in Hot-Paths erzeugt, z. B. in:
    - `crates/vorce-render/src/effect_chain_renderer/apply.rs`
    - `crates/vorce-render/src/compositor.rs`
