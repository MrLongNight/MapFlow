...
Diese Dokumentation beschreibt die Vision und den aktuellen Stand der Bevy-Integration in Vorce. Statt einer monolithischen "Bevy Scene" verfolgen wir einen modularen Ansatz, bei dem spezialisierte Bevy-Nodes nahtlos in den Vorce-Graph integriert werden.
...
1.  **Shared Engine**: Vorce startet eine einzige Bevy-Instanz im Hintergrund.
2.  **Node-to-Entity Mapping**: Jeder Bevy-Node im Vorce-Graph entspricht einer Entity oder einer Gruppe von Entities in der Bevy-World.
...
4.  **Audio-Link**: Vorce's FFT-Daten werden direkt als `AudioInputResource` in Bevy eingespeist, wo sie von spezialisierten Systemen (z.B. für das Hex-Grid) verarbeitet werden.
...
- `bevy-ui-gradients`: Inkompatibel mit 0.14. Native Vorce UI-Gradients werden bevorzugt.

## 🔜 Nächste Schritte
1.  **Node Expansion**: Implementierung der `SourceType` Varianten in `vorce-core`.
2.  **UI-Integration**: Erstellen von spezialisierten Inspector-Panels für jeden Bevy-Node.
3.  **Renderer-Optimierung**: Verbessertes Readback der Bevy-Texture in den Vorce-Main-Renderer.
