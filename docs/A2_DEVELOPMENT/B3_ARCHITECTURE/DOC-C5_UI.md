...
in favor of a custom, lightweight implementation tailored to Vorce's needs.

### Key Components

- **Canvas Types**: Located in `crates/vorce-ui/src/canvas/types.rs`.
  - `MediaPlaybackCommand`: Shared enum for media control.
  - `MediaPlayerInfo`: Shared struct for playback state.
- **Trigger System**: Located in `crates/vorce-core/src/trigger_system.rs`.
...
struct holds the state of interaction (panning, selection, dragging) while the `VorceModule`
