...
This document serves as the master plan for refactoring the Vorce application. It is broken down into specific, actionable
...
- **Input**: `crates/vorce/src/main.rs` (Settings window block)
- **Actions**:
  1. Create `crates/vorce/src/ui/mod.rs` and `crates/vorce/src/ui/settings.rs`.
...
- **Input**: `crates/vorce/src/main.rs` (`egui::SidePanel::left`)
- **Actions**:
  1. Create `crates/vorce/src/ui/sidebar.rs`.
...
- **Input**: `crates/vorce/src/main.rs` (`egui::BottomPanel`)
- **Actions**:
  1. Create `crates/vorce/src/ui/timeline.rs`.
...
- **Input**: `crates/vorce/src/main.rs` (`egui::CentralPanel`)
- **Actions**:
  1. Create `crates/vorce/src/ui/canvas.rs`.
...
- **Input**: `crates/vorce/src/main.rs` (`struct App`)
- **Actions**:
  1. Create `crates/vorce/src/state.rs`.
...
- **Input**: `crates/vorce/src/main.rs` (`impl ApplicationHandler for App`)
- **Actions**:
  1. Create `crates/vorce/src/inputs.rs`.
...
- **Input**: `crates/vorce/src/main.rs` (`App::update` method)
- **Actions**:
  1. Create `crates/vorce/src/systems/audio.rs` and `crates/vorce/src/systems/physics.rs`.
...
- **Input**: `crates/vorce/src/main.rs` (`App::render` WGPU sections)
- **Actions**:
  1. Create `crates/vorce/src/renderer.rs`.
  2. Define `struct VorceRenderer` holding Device, Queue, Surface.
