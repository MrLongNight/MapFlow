...
✅ Audio ist VERPFLICHTEND – Jeder Build muss Audio-Feature aktiviert haben.
✅ OSC ist PRIMÄR – OSC als Haupt-Control-Pfad, MIDI ist low priority.
✅ FFmpeg ist VERPFLICHTEND – Media-Playback ohne FFmpeg nicht sinnvoll.
✅ Windows + Linux – Hauptplattformen, macOS optional.
❌ Keine WebSocket-Control – OSC ist Standard in VJ-Industrie.
Kritische Arbeitspakete (in Reihenfolge):

🔴 Audio-Build-Enforcement (Backend verdrahten, UI, Tests)
🔴 OSC-Command-Schema und Integration (Routing, Feedback, Learn-Mode)
🟡 Media-Playback-State-Machine (Robustheit)
🟡 Effect-Chain-Hooks (Shader-Graph in Render-Pipeline)
🟡 Projektformat und Persistenz (Save/Load)
🟢 Multi-Window-Rendering (Phase 2 Completion)
🟢 CI/CD mit Audio und FFmpeg (Builds automatisieren)
🟢 Dokumentation und DX (Onboarding verbessern)
Nächste Schritte:

Audio-Backend-Verdrahtung starten (mapmap-core/src/audio/backend.rs erstellen)
OSC-Command-Schema dokumentieren (mapmap-control/src/osc/mod.rs erweitern)
UI-Panels für Audio und OSC erstellen (mapmap-ui/src/audio_config.rs, mapmap-ui/src/osc_config.rs)
CI/CD anpassen (Audio + FFmpeg aktivieren)
Tests schreiben und laufen lassen
