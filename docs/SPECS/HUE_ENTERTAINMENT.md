...
## 3. Architektur-Integration in VJMapper
...
*   `mapping.rs`: Konvertierung von VJMapper-Farben (RGB) in Hue-Commands.
...
3.  **Area Selection**: VJMapper lädt die in der Hue App konfigurierten "Entertainment Areas" (z.B. "Wohnzimmer", "DJ Booth").
4.  **Mapping**:
    *   **Modus A (Ambient)**: Durchschnittsfarbe des gesamten Video-Outputs wird auf alle Lampen gesendet.
    *   **Modus B (Spatial)**: Die Position der Lampen im Raum (aus Hue App) wird auf den VJMapper-Canvas gemappt. Lampe links leuchtet, wenn links im Video etwas passiert.
