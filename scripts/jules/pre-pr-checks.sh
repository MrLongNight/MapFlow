...
log "2/5: cargo clippy --workspace --all-targets --features "vorce-io/ci-linux,ffmpeg" -- -D warnings"
if ! cargo clippy --workspace --all-targets --features "vorce-io/ci-linux,ffmpeg" -- -D warnings; then
  err "Clippy hat Fehler/Warnungen (behandelt als Fehler). Bitte beheben."
  exit 3
fi
ok "Clippy OK"

log "3/5: cargo check --workspace --all-targets --features "vorce-io/ci-linux,ffmpeg""
if ! cargo check --workspace --all-targets --features "vorce-io/ci-linux,ffmpeg"; then
