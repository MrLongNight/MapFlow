...
impl App {
    ...
    pub async fn new(
        ...
        if !config.skip_autosave {
            Self::load_autosave(&mut state, &saved_config);
            Self::run_self_heal(&mut state);
        }
        ...
        let audio_backend = if config.skip_audio {
            info!("Initialization: Skipping audio backend");
            None
        } else {
            Self::init_audio(device_to_use)
        };

        // Initialize Audio Analyzer
        let audio_analyzer = vorce_core::audio::AudioAnalyzer::new(state.audio_config.clone());
        ...
        if !config.skip_mcp {
            Self::start_mcp_server(mcp_sender);
        }
        ...
        let preview_quad_buffers = Self::init_preview_quad_buffers(&mesh_renderer);

        let mut hue_controller = Self::init_hue_controller(&ui_state);
        if !config.skip_hue {
        ...
        let midi_handler = if config.skip_midi {
            info!("Initialization: Skipping MIDI");
            None
        } else {
            Self::init_midi()
        };
        ...
        let app = Self {
            ...
            layer_ping_pong,
            state,
            history: vorce_core::History::default(),
            audio_backend,
            audio_analyzer,
            audio_devices,
            egui_context,
    ...
    fn run_self_heal(state: &mut AppState) {
        ...
        for module in state.module_manager.modules() {
            for part in &module.parts {
                if let vorce_core::module::ModulePartType::Output(
                    vorce_core::module::OutputType::Projector { id, name, .. },
                ) = &part.part_type
                {
    ...
    fn init_preview_quad_buffers(
        ...
        let preview_mesh = vorce_core::Mesh {
            mesh_type: vorce_core::MeshType::Quad,
            vertices: vec![
                vorce_core::MeshVertex::new(glam::Vec2::new(0.0, 0.0), glam::Vec2::new(0.0, 0.0)),
                vorce_core::MeshVertex::new(glam::Vec2::new(1.0, 0.0), glam::Vec2::new(1.0, 0.0)),
                vorce_core::MeshVertex::new(glam::Vec2::new(1.0, 1.0), glam::Vec2::new(1.0, 1.0)),
                vorce_core::MeshVertex::new(glam::Vec2::new(0.0, 1.0), glam::Vec2::new(0.0, 1.0)),
            ],
    ...
    fn init_hue_controller(ui_state: &AppUI) -> HueController {
        ...
        HueController::new(vorce_control::hue::models::HueConfig {
