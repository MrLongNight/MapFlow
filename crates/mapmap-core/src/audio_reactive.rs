...
pub struct AudioReactiveController {
    ...
    /// Empty map for returning when disabled (avoids allocation)
    #[serde(skip, default)]
...
impl AudioReactiveController {
    ...
    pub fn update(&mut self, audio: &AudioAnalysis, current_time: f64) -> &HashMap<String, f32> {
        ...
        // Clean up previous values for removed mappings
        self.previous_values
            .retain(|k, _| self.mappings.contains_key(k));
...
impl AudioReactiveAnimationSystem {
    ...
    pub fn update(&mut self, audio: &AudioAnalysis, current_time: f64, graph: &mut ShaderGraph) {
        ...
        let final_values = Self::blend_maps(
            self.blend_mode,
            self.blend_factor,
            &animated_values,
            audio_values,
    ...
    fn blend_maps(
        ...
        for (param_path, &anim_value) in animated {
            ...
            let blended = Self::calculate_blended_value(
                blend_mode,
                blend_factor,
                anim_value,
                audio_value,
        ...
        for (param_path, &audio_value) in audio {
            if !animated.contains_key(param_path) {
                // anim_value is 0.0
                let blended = Self::calculate_blended_value(blend_mode, blend_factor, 0.0, audio_value);
                result.insert(param_path.clone(), blended);
            }
        }
    ...
    #[inline]
    fn calculate_blended_value(
        mode: AudioAnimationBlendMode,
        factor: f32,
        anim_value: f32,
        audio_value: f32,
    ) -> f32 {
        match mode {
            AudioAnimationBlendMode::Replace => {
                anim_value * (1.0 - factor) + audio_value * factor
            }
            AudioAnimationBlendMode::Add => anim_value + audio_value * factor,
            AudioAnimationBlendMode::Multiply => {
                anim_value * (1.0 + (audio_value - 1.0) * factor)
            }
        }
    }
}
...
mod tests {
    ...
    fn test_blend_modes() {
        ...
        let blended = AudioReactiveAnimationSystem::blend_maps(
            system.blend_mode,
            system.blend_factor,
            &animated,
            &audio,
