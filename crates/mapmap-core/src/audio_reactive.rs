...
mod tests {
    ...
    #[test]
    fn test_smoothing_attack_release() {
        let mut controller = AudioReactiveController::new();
        let mapping = AudioReactiveMapping {
            ...
            output_min: 0.0,
            output_max: 1.0,
            smoothing: 0.0, // No base smoothing
            attack: 0.1,    // 100ms to reach full value
            release: 0.2,   // 200ms to fall to zero
        };
        controller.add_mapping("node.test".to_string(), mapping);
