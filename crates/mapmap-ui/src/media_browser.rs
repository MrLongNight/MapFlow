...
impl MediaBrowser {
    ...
    pub fn ui(
        ...
        crate::widgets::panel_container(ui, &locale.t("panel-media-browser"), |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                let path_response = ui.add(
                    egui::TextEdit::singleline(&mut self.path_input)
                        .desired_width(ui.available_width() - 40.0) // Space for Close button
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if ui.button("✕").on_hover_text("Close Browser").clicked() {
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                if entry_indices.is_empty() {
                    ...
                    match self.view_mode {
                        ViewMode::Grid => {
                            if let Some(act) = self.render_grid_view(ui, &entry_indices, icons) {
                                action = Some(act);
                            }
                        }
                        ViewMode::List => {
                            if let Some(act) = self.render_list_view(ui, &entry_indices, icons) {
                                action = Some(act);
                            }
                        }
                    }
