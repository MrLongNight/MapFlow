...
use egui::Ui;
use mapmap_core::module::{BlendModeType, LayerType, MaskShape, MaskType, ModulePartId};
...
pub fn render_layer_ui(
    ...
    preview_context: &super::InspectorPreviewContext,
) {
    ui.label("📋 Layer:");

    ui.separator();
    super::common::render_inspector_preview_toggle(canvas, ui);
    super::common::render_layer_preview_panel(canvas, ui, module_id, part_id, preview_context);
    ui.separator();

    // Helper to render mesh UI
    let mut render_mesh_ui =
        |ui: &mut Ui, mesh: &mut mapmap_core::module::MeshType, id_salt: u64| {
            super::mesh::render_mesh_editor_ui(
                mesh_editor,
                last_mesh_edit_id,
                ui,
                mesh,
