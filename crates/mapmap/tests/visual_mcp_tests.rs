...
fn test_mcp_visual_capture() {
    ...
    if !runner_status.success() {
        println!("Warning: mcp_test_runner exited with a non-zero status.");
        if std::env::var("CI").is_err() {
            // In a local environment, this is a real failure.
    ...
    // Wait for the file to be completely written just in case
    std::thread::sleep(std::time::Duration::from_millis(500));

    if !actual_path.exists() {
        panic!("The actual image was not generated by mcp_test_runner. Test failed.");
    }
