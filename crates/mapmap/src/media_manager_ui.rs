use egui::{Ui, Color32, Sense, Vec2, Id};
use mapmap_core::media_library::{MediaLibrary, MediaType, MediaItem};
    ...
    fn render_sidebar(&mut self, ui: &mut Ui, library: &mut MediaLibrary) {
        ui.vertical(|ui| {
            ...
            for playlist in &library.playlists {
                ui.horizontal(|ui| {
                    if ui.selectable_label(self.selected_playlist.as_ref() == Some(&playlist.name), &playlist.name).clicked() {
        ...
        ui.vertical(|ui| {
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                let items = if let Some(playlist_name) = &self.selected_playlist {
                    if let Some(playlist) = library.playlists.iter().find(|p| &p.name == playlist_name) {
                        playlist.items.iter().filter_map(|path| library.items.get(path)).collect::<Vec<_>>()
                    } else {
                        vec![]
                    }
                } else {
                ...
                let filtered_items: Vec<&MediaItem> = items.into_iter().filter(|item| {
        ...
        egui::Grid::new("media_grid").striped(true).show(ui, |ui| {
            for (i, item) in items.iter().enumerate() {
                ...
                if ui.is_rect_visible(rect) {
                    ...
                    ui.painter().text(rect.center(), egui::Align2::CENTER_CENTER, icon, egui::FontId::proportional(size.y * 0.5), Color32::WHITE);

                    // Name
                    ui.painter().text(rect.center() + Vec2::new(0.0, size.y * 0.4), egui::Align2::CENTER_BOTTOM, &item.name, egui::FontId::proportional(12.0), Color32::WHITE);

                    // Hover effect
                    if response.hovered() {
                        ui.painter().rect_stroke(rect, 2.0, egui::Stroke::new(2.0, Color32::LIGHT_BLUE), egui::StrokeKind::Outside);
                    }
                }
